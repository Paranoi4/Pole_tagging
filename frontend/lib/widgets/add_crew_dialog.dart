// 📁 lib/widgets/add_crew_dialog.dart
//
// Opened from the "Crew" nav tile on the home screen. Lets an Admin add one
// or more crews without leaving the dialog ("+ Add another" resets the form
...
/// Shows the Add Crew dialog. Call this from the Crew nav tile's onTap.
Future<void> showAddCrewDialog(BuildContext context) {
  return showDialog(
    context: context,
    builder: (_) => const AddCrewDialog(),
  );
}

class AddCrewDialog extends ConsumerStatefulWidget {
  const AddCrewDialog({super.key});

  @override
  ConsumerState<AddCrewDialog> createState() => _AddCrewDialogState();
}

class _AddCrewDialogState extends ConsumerState<AddCrewDialog> {
  ...
  Future<void> _submit({required bool addAnother}) async {
    ...
    try {
      await ref.read(apiProvider).createCrew(
            crewLabel: _nameController.text.trim(),
